...
class Cloud {
  ...
  publishAndForget = (topic, message) => {
    this.#iotCoreDevice.publish(topic, JSON.stringify(message), { qos: 0 }, function(err) {
      if (err) console.log(err)
    })
  }

  publish = (topic, message) => {
    this.#iotCoreDevice.publish(topic, JSON.stringify(message), { qos: 1 }, function(err) {
      if (err) console.log(err)
    })
  }
  ...
  publishUnformattedPromised = async (topic, message) => {
    await this.#iotCoreDevice.publish(topic, message, { qos: 1 }, function(err) {
      if (err) {
        console.log(err)
        return false
      }
  ...
  uploadSingleLapTelemetry = (topic, lapPayload, laptimeInMs, maxLaptime) => {
    ...
    let iot = this.#iotCoreDevice
		...
		if (lapDuration > maxLaptime) console.log(`Skipping ${laptimeInMs} - duration too long: ${lapDuration}`)
          ...
          files.forEach(function (file) {
            try {
              if (file.match(/summary*/)) {
                ...
                let summaryDBFile = JSON.parse(fs.readFileSync(`${dataDirectory}/${file}`, 'utf-8'))
                let summaryDBFileStringified = JSON.stringify(fs.readFileSync(`${dataDirectory}/${file}`, 'utf-8'))
                ...
                if (!summaryDBFile.published && summaryDBFile.lapTimes.length > 0) {
                  console.log(sessionID + " publishing summary!")
                  iot.publish("trackdata-sessionsummary", summaryDBFileStringified, { qos: 1 }, function(err) {
                ...
                for (let i=0; i<summaryDBFile.lapTimes.length; i++) {
                  setTimeout(() => {
                    ...
                    // don't upload laptimes longer than 5 minutes, one day if we get to nurburgring this will need to be FIXED!!!
                    if (lapDuration > maxLaptime) console.log(`Skipping ${lapFilePath} - duration too long: ${lapDuration}`)

                    if (lapS3 == false && lapDuration < maxLaptime) {
                      try {
                      UploadedToCloud++
                      lapPayload = JSON.stringify(fs.readFileSync(`${lapFilePath}`, 'utf-8'))

                      iot.publish(topic, lapPayload, { qos: 1 }, function(err) {
                        if (err) console.log(err)
                        else {
                          try {
                          console.log(summaryDBFile.lapTimes[i])
                          summaryDBFile.lapTimes[i].uploaded_to_s3 = true
                          console.log(`[CLOUD] - ${lapFilePath} uploaded to cloud successfully!`)
                          fs.writeFileSync(`${summaryFilePath}`, JSON.stringify(summaryDBFile))
                          } catch(e) { console.log(e)}
                        }
                      }) } catch(e1) { console.log(e1) }
                    ...
                    if (i == summaryDBFile.lapTimes.length-1 && UploadedToCloud == 0) {
  ...
  uploadToCloud = (dataDirectory, topic) => {
    ...
    let moveFile = this.#moveFile
      ...
      let jsonSessionsProcessed = []
            ...
            iot.publish('trackdata-uncompressed', JSON.stringify(fileContent), { qos: 1 }, function(err) {
              if (err) console.log(err)
              else {
                try {
                  if (fileContentJSON.length > 40 ) { //ignore empty sessions
                    let payload = {
                ...
                console.log(`Upload of ${dataDirectory}/${file} - successful!`)
                i++

                if (!file.match(/summary*/)) {
                  moveFile(`${dataDirectory}/${file}`, `/home/fd27845fce8446287de9506c4650c71/.bin/archive/${file}`)
                }

                if (i == filesAmount) {
                  let uniqueSessionsProcessed = _.uniq(sessionsProcessed, x => x.session_id);
                  console.log("Done")
                    ...
                    uniqueSessionsProcessed.forEach(function (session){
                      try {
                          ...
                          if (err) console.log(err)
                            ...
                            let summaryContentJSON = JSON.parse(summaryContent)

                            iot.publish('trackdata-sessionsummary', JSON.stringify(summaryContent), { qos: 1 }, function(err) {
                              if(err) console.log(err)
                              else {
                                console.log(`Upload of ${dataDirectory}/summary-${session.session_id}.dat - successful!`)
                                // archive all session files + summary files
                                moveFile(`${dataDirectory}/summary-${session.session_id}.dat`, `/home/fd27845fce8446287de9506c4650c7/.bin/archive/summary-${session.session_id}.dat`)
                              }
                            })
                          }
                        })
