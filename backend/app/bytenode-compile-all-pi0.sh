#!/bin/bash
#####
### This scripts compiles binaries for pi0 W1
###
####

export PATH=$PATH:/usr/local/lib/node/node-v12.18.2-linux-arm64/bin
...
cp -r start-be-gps-pi0.sh dist/start-be-gps.sh
cp -r s-s.sh dist/
cp -r firewall.sh dist/
cp -r get-updates-pi0.sh dist/get-updates.sh
...
rm app.js
tar -czvf rh-backend-pi0.tar.gz dist/
...
cp get-updates-pi0.sh rh-backend-pi0.tar.gz ~/.bin/release/backend/
mv ~/.bin/release/backend/get-updates-pi0.sh ~/.bin/release/backend/get-updates.sh
