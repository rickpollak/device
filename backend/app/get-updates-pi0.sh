...
scp -P9412 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no rhero@sftp.racetrackhero.com:~/rh-backend-pi0.tar.gz ~/.bin/release/backend

RESULT=$?

if [ $RESULT -eq 0 ]; then
  NEW_MD5=`md5sum ~/.bin/release/backend/rh-backend-pi0.tar.gz | cut -d " " -f1`
