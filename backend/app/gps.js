...
const version = "1.10"
const fs = require('fs')
const MY_ID = fs.readFileSync('/var/lib/dbus/machine-id').toString().replace(/\n/g, '')
...
const directoryRamFSPath = '/home/fd27845fce8446287de9506c4650c71/.bin/data-ramfs'
const directoryCloudPath = '/home/fd27845fce8446287de9506c4650c71/.bin/cloud-data'
const directoryReleasePath = '/home/fd27845fce8446287de9506c4650c71/.bin/release'
...
const cloud = new Cloud([`bootbeat/updates/${MY_ID}/racetrack`, `bootbeat/updates/${MY_ID}/config`, `bootbeat/updates/${MY_ID}/scoreboard`, `bootbeat/bootstrap/${MY_ID}`], MY_ID)
const compression = new Compress()
const math = new LaptimeMath()

// CONFIG file
const configFilePath = '/home/fd27845fce8446287de9506c4650c71/.bin/cloud-data/config.json'
const racetrackFile = '/home/fd27845fce8446287de9506c4650c71/.bin/cloud-data/racetracks.json';
...
function saveLapToDisk(l_id, l_number, l_time, s_id, ignore) {
  ...
  if (!ignore) {
    ...
    if (!config.silent_mode) {
      ...
      cloud.uploadSingleLapTelemetry("telemetry-lapdata", compressed, lap_timeInMs, config.max_upload_laptime_length)
    }
	}
	tmpLog = [];
  ...
  gps.getReadings().on('event', (gpsCurrentReading) => {
    ...
    if (!gps_locationEstablished) {
      ...
      if (tmpLocationData) {
        ...
        cloud.publish("heartbeat", `{"trackerId": "${MY_ID}", "racetrackId": "${gps_racetrackId}"}`)

        cloud.publish("bootbeat", `{"tracker_id": "${MY_ID}", "message": "scoreboardPlease", "racetrackID": "${gps_racetrackId}" }`)
    ...
    if (sessionPending) {
      if (gps_drawing_counter > 10 && gps_drawing_need_to_draw) { // 10*100ms = 1s update screen every second
        socket.publish({ type: "lineUpdate", payload: publishable_payload }, "gpsLight")
        gps_drawing_counter = 0
      }

      if (gps_drawing_counter > 10 && !gps_drawing_need_to_draw) {
        socket.publish({ type: "markerUpdate", payload: publishable_payload }, "gpsLight")
        gps_drawing_counter = 0
      }
    }
    ...
    // ** SESSION STARTS
    if (vehicleNotAtRestCounter >= 5 && !sessionPending && !sessionId && gps_lat && gps_lon) {
      // stop uploading any existing data to the cloud, we need full processing power to measure laptimes and telemetry
      try { cloud.stopUploadTimer() } catch (e) { console.log(e) }
    ...
    if (sessionPending && sessionId) {
      ...
      if (!gps_lapStartTimestamp) gps_lapStartTimestamp = gpsCurrentReading.time

      driveLog.push(payload)

      if (gpsLaptimer.didCrossLineV2(geoLibCords, gps_finishLine) && !gps_crossLine_hold) {
        ...
        } else { // SECOND LAP AND OTHER LAPS OVER 2
          gps_crossLine_hold = true
...
socket.get().on('event', (d, d1, d2) => {
  ...
  if (d == 'config' && d1 == 'wifiConnect' && d2) {
    ...
    if (ssid && passphrase) {
      console.log(`Connecting to ${ssid} with ${passphrase}`)
      sys.wifiConnect(ssid, passphrase).then(() => {
  ...
  if (d == 'config' && d1 == 'getConfig') {
    ...
    if (gps_locationEstablished) socket.publish(`Welcome to ${gps_raceTrackName}`,"barMessage")
...
cloud.get().on('event', (msg, payload) => {
  ...
  if (msg == `bootbeat/bootstrap/${MY_ID}`) {
    ...
    cloud.publish(`bootbeat`, `{"tracker_id": "${MY_ID}", "message": "thisIsMyConfig", "config": ${formattedConfig}}`)
  }

  if (msg == `bootbeat/updates/${MY_ID}/racetrack`) { // receiving updated racetrack db.
    ...
    push = async () => {
      ...
      await cloud.publishPromised("bootbeat", `{"tracker_id": "${MY_ID}", "message": "updatedRacetrackDB"}`)
  ...
  if (msg == `bootbeat/updates/${MY_ID}/scoreboard`) { // receiving updated racetrack db.
  ...
  if (msg == `bootbeat/updates/${MY_ID}/config`) { // receiving updated config when device was offline and new settings have been posted to the cloud.
    ...
    console.log(`[SOFTWARE UPDATES] - Checking - our config version: ${backend_cur_ver} vs incoming cloud version: ${backend_new_ver}`)
    ...
    if (backend_cur_ver !== backend_new_ver) {
      ...
      sys.updateSoftware(`${directoryReleasePath}/backend`, "backend", backend_new_ver)
    } else {
      console.log(`[SOFTWARE UPDATES] - NO NEW UPDATES.`)
    }


    cloud.publish("bootbeat", `{"tracker_id": "${MY_ID}", "message": "updatedConfig"}`) //ack-ed and send answer to cloud
  }
  ...
  if (msg == "connected") {
    ...
    const timeoutObj1 = setTimeout(() => {
      cloud.publish("bootbeat", `{"tracker_id": "${MY_ID}", "message": "amINewHere?"}`)
      console.log('[CLOUD] - asking if I am new to the system.');
    }, 200);

    const timeoutObj2 = setTimeout(() => {
      cloud.publish("bootbeat", `{"tracker_id": "${MY_ID}", "message": "anyupdates?"}`)
...
// -> comment out
cloud.uploadAllSessionTelemetry("telemetry-lapdata", directoryDataPath, config.max_upload_laptime_length)
...
console.log(`******************RACETRACKHERO VERSION: ${version}****************************`)
